...
const Featured = () => {
  const data = useStaticQuery(graphql`
    {
      featuredAI: allMarkdownRemark(
        filter: { fileAbsolutePath: { regex: "/content/featured-ai/" } }
      ...
      featuredWeb: allMarkdownRemark(
        filter: { fileAbsolutePath: { regex: "/content/featured-web/" } }
        sort: { fields: [frontmatter___date], order: ASC }
      ) {
        edges {
          node {
  ...
  return (
    <div>
      ...
      <section id="web-projects">
        ...
        <StyledProjectsGrid>
          {featuredProjectsWeb &&
            featuredProjectsWeb.map(({ node }, i) => {
              const { frontmatter, html } = node;
              const { external, title, tech, github, cover, cta } = frontmatter;
              const image = getImage(cover);
